...
func (s *Server) Start() error {
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if ok {
			...
			logger.Info(fmt.Printf("user: %s ,oldConnet address: %v \n", oldConnectUser.Connect.RemoteAddr(), "\n"))
		}
		// 独立协程处理
		go func(user string, conn net.Conn) error {
			for {
				...
				logger.Info(fmt.Printf("user:%s , %s , Header : %v \n", user, time.Now().Format(TimeFormatLayOut), readFrame.Header))
				// 正常数据
				// 是否采用的掩码
